...
export default async function TrendingThisWeek() {
    ...
    const data = await fetch('https://api.themoviedb.org/3/trending/all/week', {
        method: 'GET',
        headers: {
    ...
    const trendingThisWeek = dataJson.results.filter((item: TrendingMovieOrTVShow) => item.media_type === 'movie' || item.media_type === 'tv');

    const cards = trendingThisWeek.map((item: TrendingMovieOrTVShow, index: number) => (
        <Card
            key={item.id}
            card={{
